...
ClientsRecords.checks = {
    ...
    // cross checks
    // make sure that a client which needs a secret (no resource owner can be sollicited) has at least one
    // the auth method requires a shared secret or there is no resource owner involved in the authorization flow
    //  data expects:
    //  - entity: an { entity, record } object
    async crossCheckSecrets( data, opts={} ){
        let result = [];
        const hasResourceOwner = GrantType.hasResourceOwner( data.entity.record.grant_types );
        const def = AuthMethod.byId( data.entity.record.token_endpoint_auth_method );
        if( !hasResourceOwner || ( def && AuthMethod.needsSecret( def ))){
            const secrets = ClientSecrets.activeSecrets( data.entity );
            if( !secrets.length ){
                return new new TM.TypedMessage({
                    level: TM.MessageLevel.C.ERROR,
                    message: pwixI18n.label( I18N, 'clients.checks.secret_unset' )
                });
            }
        }
        return null;
