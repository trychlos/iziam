/*
 * /imports/client/components/clients_groups_edit_dialog/clients_groups_edit_dialog.js
 *
 * Display the groups tree as an editable component with dnD and delete features.
 * 
 * +- <this>
 *     |
 *     +- clients_groups_hierarchy_pane
 *         |
 *         +- groups_tree
 *         |
 *         +- groups_buttons
...
import '/imports/client/components/clients_groups_hierarchy_pane/clients_groups_hierarchy_pane.js';
...
Template.clients_groups_edit_dialog.onCreated( function(){
    ...
    self.autorun(() => {
        ...
        if( organization ){
            self.APP.groups.set( _.cloneDeep( organization.DYN.clients_groups.get()));
        }
    });
...
Template.clients_groups_edit_dialog.onRendered( function(){
    const self = this;

    // whether we are running inside of a Modal
    self.autorun(() => {
        self.APP.isModal.set( self.$( '.c-clients-groups-edit-dialog' ).parent().hasClass( 'modal-body' ));
    });
    ...
    self.autorun(() => {
        if( self.APP.isModal.get()){
            Modal.set({
                target: self.$( '.c-clients-groups-edit-dialog' )
            });
        }
    });
});

Template.clients_groups_edit_dialog.events({
    ...
    'md-click .c-clients-groups-edit-dialog'( event, instance, data ){
    ...
    'iz-submit .c-clients-groups-edit-dialog'( event, instance ){
        ...
        Meteor.callAsync( 'clients_groups.upsert_tree', organizationId, groups )
            .then(() => {
