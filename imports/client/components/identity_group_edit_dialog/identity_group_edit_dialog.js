/*
 * /imports/group/components/identity_group_edit_dialog/identity_group_edit_dialog.js
 *
...
import '/imports/client/components/identity_group_properties_pane/identity_group_properties_pane.js';

import './identity_group_edit_dialog.html';

Template.identity_group_edit_dialog.onCreated( function(){
    const self = this;
    //console.debug( this );
...
Template.identity_group_edit_dialog.onRendered( function(){
    const self = this;

    // whether we are running inside of a Modal
    self.autorun(() => {
        self.APP.isModal.set( self.$( '.c-identity-group-edit-dialog' ).parent().hasClass( 'modal-body' ));
    });
    ...
    self.autorun(() => {
        if( self.APP.isModal.get()){
            Modal.set({
                target: self.$( '.c-identity-group-edit-dialog' )
            });
        }
    });
...
Template.identity_group_edit_dialog.helpers({
...
Template.identity_group_edit_dialog.events({
    // submit
    //  event triggered in case of a modal
    'md-click .c-identity-group-edit-dialog'( event, instance, data ){
    ...
    // submit
    'iz-submit .c-identity-group-edit-dialog'( event, instance ){
        ...
        const closeFn = function(){
            if( instance.APP.isModal.get()){
                ...
                instance.$( '.c-identity-group-properties-panel' ).trigger( 'iz-clear-panel' );
                instance.$( '.NotesEdit' ).trigger( 'iz-clear-panel' );
            }
        };
        if( this.targetDatabase !== false ){
            Meteor.callAsync( 'identities_groups.upsert_item', this.organization._id, item )
                .then(() => {
